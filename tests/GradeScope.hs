...
module GradeScope ( visibility, Visibility(..), makeVisible
                  , scored
...
scoreTests outfile opts tests = \testStatus -> do
  ...
  Const summary <- flip State.evalStateT traversalStInit $ getCompose $ getTraversal $
...
instance JSON TestResult where
  ...
  showJSON tr@TestResult{..} =
    ...
    where
      output =
        case (outcome, Runners.resultDescription resultMetadata) of
          (Runners.Success, "") -> "Success !"
          (_, s)                -> s

      outcome = Runners.resultOutcome resultMetadata
...
data TraversalSt = TraversalSt { tScore :: Int
                               , tPath  :: String
                               }

type ScoreTraversal = Traversal (Compose (State.StateT TraversalSt IO) (Const ScoreSummary))
...
scoreSingleTest statusMap options resultName0 _ = Traversal $ Compose $ do
  resultId <- tScore <$> State.get
  ...
  resultName <- (++ resultName0) . tPath <$> State.get
  ...
  Const summary <$ State.modify traversalStIncr
  where
...
scoreGroup group kids = Traversal $ Compose $ do
  currentPath <- State.gets tPath
  State.modify (traversalStPutPath (currentPath ++ group ++ "."))
  result <- getCompose $ getTraversal kids
  State.modify (traversalStPutPath currentPath)
  return result

traversalStIncr :: TraversalSt -> TraversalSt
traversalStIncr (TraversalSt{..}) =
  TraversalSt { tScore = tScore + 1
              , ..
              }

traversalStPutPath :: String -> TraversalSt -> TraversalSt
traversalStPutPath p (TraversalSt{..}) =
  TraversalSt { tPath = p
              , ..
              }

traversalStInit :: TraversalSt
traversalStInit =
  TraversalSt { tScore = 0
              , tPath  = ""
              }
